...
function buildSettings(){
  const pfs=[
    ...
    {id:'inst',icon:'⚡',name:'Instantly',bg:'#F5F3FF',fields:[{k:'instantly',l:'API Key',ph:'inst_xxxxxxxxxxxx',hint:'<a href="https://app.instantly.ai/app/settings/integrations" target="_blank">Instantly</a> → API'}]},
    {id:'mon',icon:'📋',name:'Monday.com',bg:'#ECFEFF',fields:[{k:'monday',l:'API Token',ph:'eyJhbGciOiJ…',hint:'<a href="https://monday.com/apps/manage/tokens" target="_blank">Monday</a> → Developers'}]},
  ];
...
function svS(id){
  const m={anthropic:{ak:'sf-ak'},g:{clientId:'sf-clientId',ga4:'sf-ga4',gsc:'sf-gsc',ads:'sf-ads',adsToken:'sf-adsToken'},li:{liId:'sf-liId',liSecret:'sf-liSecret',liOrg:'sf-liOrg',liToken:'sf-liToken'},inst:{instantly:'sf-instantly'},mon:{monday:'sf-monday'}};
...
function clrS(id){const m={anthropic:['ak'],g:['clientId','ga4','gsc','ads'],li:['liId','liSecret','liOrg','liToken'],inst:['instantly'],mon:['monday']};(m[id]||[]).forEach(k=>CFG[k]='');localStorage.setItem(CK,JSON.stringify(CFG));buildSettings();loadAll();}
