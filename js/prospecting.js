...
async function prEnrichClaude(inst) {
  if (typeof CFG === 'undefined' || !CFG.ak) return [];
  var prompt = 'Busca en internet el nombre, cargo y email del responsable de empleabilidad o career services de: '
    + inst.name + ' (' + PR_COUNTRY_NAMES[inst.country] + ', dominio: ' + inst.domain + ').\n\n'
    + 'Responde SOLO con JSON válido, sin markdown:\n'
    + '{"contacts":[{"name":"Nombre","role":"Cargo","email":"email o vacio","linkedin":"URL o vacio","confidence":50}]}\n'
    + 'Si no encuentras nada: {"contacts":[]}. Máximo 3 contactos.';
  try {
    ...
    return (json.contacts||[]).map(function(c){ c.source='Claude IA'; return c; });
  } catch(e) { return []; }
}
...
async function prPitch(instId) {
  ...
  if (!inst || typeof CFG==='undefined' || !CFG.ak) return;
  var btn = document.querySelector('[onclick="prPitch(\'' + instId + '\')"]');
  if (btn) { btn.disabled=true; btn.textContent='Generando…'; }

  var identity = (typeof biGetContext==='function') ? biGetContext() : '';
  ...
  var prompt = identity + '\n\nEscribe un email de outreach en español para ' + to + ' de ' + inst.name + '.\n'
    + 'OBJETIVO: proponer alianza donde la institución recomiende Próximo Rol a sus alumnos para preparar entrevistas y mejorar empleabilidad.\n'
    + 'Asunto: máx 60 chars. Cuerpo: 120-180 palabras. Tono profesional pero cercano.\n'
    + 'Propón una llamada de 20 min como siguiente paso. Firma como equipo Próximo Rol.\n'
    + 'Formato: ASUNTO: ...\n\n[cuerpo]. Sin markdown.';

  try {
    var data = await antFetch({ model:'claude-sonnet-4-20250514', max_tokens:400, messages:[{role:'user',content:prompt}] });
    var pitch = (data.content||[]).filter(function(b){return b.type==='text';}).map(function(b){return b.text;}).join('');
