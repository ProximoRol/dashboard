...
function rvLoad() {
  if (window.__RV_CACHE__) return window.__RV_CACHE__;
  try { return JSON.parse(localStorage.getItem(RV_KEY) || 'null') || rvDefault(); }
  catch (_) { return rvDefault(); }
}
async function rvLoadAsync() {
  const data = await DB.get('review','data', null);
  window.__RV_CACHE__ = data || rvDefault();
  return window.__RV_CACHE__;
}
function rvSave(d) {
  window.__RV_CACHE__ = d;
  DB.set('review','data', d);
}
function rvDefault() {
  return {
...
async function rvRunWeekly() {
  ...
  const rv    = await rvLoadAsync();
  const mem   = typeof memLoad === 'function' ? memLoad() : {};
  const buf   = window.__CP_BUF_CACHE__ || JSON.parse(localStorage.getItem('pr_session_buffer_v1') || '[]');
...
Sé directo. Sin introducciones. En español.`;
  ...
  try {
    const data = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'review', action:'weekly', payload:{ metrics, sessionSummary, memProfile, pendingRecs, failedRecs, sinceDate, userCurrency: typeof getUserCurrency === 'function' ? getUserCurrency() : null } } });
    ...
    localStorage.removeItem('pr_session_buffer_v1'); // limpiar legacy también
...
En español. Sin introducciones. Máximo 500 palabras.`;
  ...
  try {
    const data = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'review', action:'monthly', payload:{ metrics, pnlContext: pnlCtx, lastWeeklies, memProfile, failedRecs: (mem.recommendations?.failed || []).map(r => r.text).slice(0,3).join(' | '), userCurrency: typeof getUserCurrency === 'function' ? getUserCurrency() : null } } });
    const text = data.text || '';
...
async function rvRenderHistory() {
