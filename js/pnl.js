...
function pnlComputeMonth(m, data) {
  ...
  /* Budget / OpEx — lee TODOS los gastos del mes directamente desde eco_actual_v2.
     No dependemos de BGT_DATA.rows (que puede tener datos hardcodeados de otro cliente
     si el usuario no ha visitado la página de Budget en esta sesión).
     Formato de key: "cat|linea|mes" o "_libre|desc|mes" — sumamos todo lo que termine en "|m" */
  const actual = pnlGetActualCosts();
  let opex_actual = 0;
  Object.entries(actual).forEach(([k, v]) => {
    const lastPipe = k.lastIndexOf('|');
    if (lastPipe !== -1 && k.slice(lastPipe + 1) === String(m)) {
      opex_actual += parseFloat(v) || 0;
    }
  });
