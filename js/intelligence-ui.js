/* ═══════════════════════════════════════════════
   INTELLIGENCE UI — GA4 Intel, Ads AI, KWI, SERP,
   Keyword Economics, SEO, Info Modal, Reports
   AI calls go through backend proxy.
   Depends on: core.js (apiFetch, API_BASE), analytics.js
   ═══════════════════════════════════════════════ */
...
Da 6 recomendaciones, 7 errores/issues, 10 keywords en español adaptadas al mercado hispanohablante. Prioriza España y LATAM.`;
    ...
    const data = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'ga4-intel', payload:{ ga4Data } } });
    
    if(data.recommendations?.length) renderRecommendations(data.recommendations);
    if(data.errors?.length)          renderErrors(data.errors);
    if(data.keywords?.length)        renderKeywords(data.keywords);
...
/* ══════════════════════════════════════════════════════
   ADS AI RECOMMENDATIONS — MOVIDO A paid-media-ui.js
   runAdsAIRecommendations() → runPaidMediaAI()
   ══════════════════════════════════════════════════════ */
...
{"keywords":[{"kw":"...","volume":"low|medium|high","cpc":"£X-£Y or €X-€Y or $X-$Y","competition":"low|medium|high","strategy":"organic|paid|both","market":"...","intent":"...","rationale":"..."}]}`;
    ...
    const parsed = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'kwi-analysis', payload:{ markets, focus } } });
...
Return ONLY valid JSON, no markdown, no preamble:
{
  ...
  try{
    const r = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'serp-analysis', payload:{ query, market } } });
...
Provide a comprehensive economic analysis. Return ONLY valid JSON, no markdown:
{
  ...
  try{
    const r = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'econ-analysis', payload:{ keyword: kw, budget, cvr, market } } });
...
Return ONLY valid JSON, no markdown:
{
  ...
  try{
    const r = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'page-audit', payload:{ url, focus } } });
...
async function runSEOChat(){
  ...
  try{
    const data = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'seo-chat', payload:{ messages: msgs } } });
...
Return ONLY valid JSON, no markdown:
{
  ...
  try{
    const r = await apiFetch('/api/claude-proxy', { method:'POST', body:{ module:'intelligence', action:'competitor-spy', payload:{ spyUrl: url, vsUrl } } });
